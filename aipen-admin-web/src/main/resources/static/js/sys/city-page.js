...
//    authtree.render('#LAY-auth-tree-index', treeJson, 'ids[]');
    ...
    $(".layui-form-label").css("width","150px","text-align","center");
    $(".layui-input").css("width","180px");
    // 数据提交至后台保存
    form.on("submit(save)", function(data){
        ...
        top.asyncRequest({
            ...
            data: {
                id : data.field.id,
                agentNo : data.field.agentNo
            },
            dataType: "json",
            success: function (result) {
                if(result.success){
                    tips('保存成功', true, true);
                    parent.layui.table.reload('roleTable',{page:{curr:$(".layui-laypage-em").next().html()}});
